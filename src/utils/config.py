...
class AppConfig:
    ...
    def from_env(cls, env: str | None = None) -> AppConfig:
        ...
        logger.debug(
            "Chargement de la configuration", extra={"env": resolved_env, "path": str(config_path)}
        )
    ...
    def _build(cls, path: Path, env: str) -> AppConfig:
        ...
        spark_cfg = SparkConfig(
            app_name=os.getenv("SPARK_APP_NAME", spark_raw.get("app_name", "spark-project")),
            master=os.getenv("SPARK_MASTER", spark_raw.get("master", "local[*]")),
            log_level=os.getenv("SPARK_LOG_LEVEL", spark_raw.get("log_level", "WARN")),
        ...
        storage_cfg = StorageConfig(
            data_lake_path=os.getenv("DATA_LAKE_PATH", storage_raw.get("data_lake_path", "")),
            checkpoint_path=os.getenv("CHECKPOINT_PATH", storage_raw.get("checkpoint_path", "")),
